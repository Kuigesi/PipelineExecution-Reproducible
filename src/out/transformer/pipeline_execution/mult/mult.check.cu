...
void Snippet(int x0) {
  ...
  if (x15 >= 0 && x15 < 2) {
    ...
    // end initializing fixed GPU array of size 512 and type Float and device (pre-rename) x66
    // begin initializing fixed GPU array of size 1536 and type Float and device (pre-rename) x66
    CUDA_CALL(cudaSetDevice(x15));
    float* x27 = (float*)malloc(0 * sizeof(float));
    CUDA_CALL(cudaMalloc(&x27, (size_t)(1536 * sizeof(float))));
    x20<<<dim3(28, 1, 1), dim3(512, 1, 1)>>>(x27, 0, 1536);
    // end initializing fixed GPU array of size 1536 and type Float and device (pre-rename) x66
    int x28 = 0;
    int x37 = x15 + 2;
    while (x28 != 3) {
      int x54 = 0;
  ...
  if (x15 >= 2 && x15 < 4) {
    ...
    CUDA_CALL(cudaMalloc(&x63, (size_t)(1536 * sizeof(float))));
    x20<<<dim3(28, 1, 1), dim3(512, 1, 1)>>>(x63, 0, 1536);
    // end initializing fixed GPU array of size 1536 and type Float and device (pre-rename) x66
    // begin initializing fixed GPU array of size 1536 and type Float and device (pre-rename) x66
    CUDA_CALL(cudaSetDevice(x15));
    float* x64 = (float*)malloc(0 * sizeof(float));
    CUDA_CALL(cudaMalloc(&x64, (size_t)(1536 * sizeof(float))));
    x20<<<dim3(28, 1, 1), dim3(512, 1, 1)>>>(x64, 0, 1536);
    // end initializing fixed GPU array of size 1536 and type Float and device (pre-rename) x66
    int x65 = 0;
    int x66 = x15 - 2;
    while (x65 != 3) {
